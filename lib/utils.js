...
async function processManyToManyRelation(request, response, context, adminResources, relation, relationRecord) {
...
async function processRelation(request, response, context, adminResources, relation) {
    ...
    if (relation.relation.type === 'many-to-many') {
        await Promise.all(relation.records.map(rr => processManyToManyRelation(request, response, context, adminResources, relation, rr)));
    } else {
...
async function processRecord(request, response, context, record, adminResources) {
    try {
        ...
        if (record.relations) {
            await Promise.all(
                record.relations.map(relation => processRelation(request, response, context, adminResources, relation))
            );
        }
