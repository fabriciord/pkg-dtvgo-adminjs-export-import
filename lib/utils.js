...
async function processRecord(record, adminResources) {
    try {
...
export const saveRecords = async (records, resource) => {
    ...
    return Promise.all(records.map(record => processRecord(record, adminResources)));
};
...
export const sortObjectByCustomOrder = (object) => {
    return ['_id', 'title', 'name', 'description', 'categoria', 'subcategoria', 'type', 'titulo']
